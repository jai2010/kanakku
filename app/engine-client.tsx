...
export function EngineClient(): JSX.Element {
  ...
  function showDemos(): void {
    ...
    window.requestAnimationFrame(() => {
      document.getElementById('engine-demos')?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
    });
  }
  ...
  return (
    <div className={`app${busy ? ' busy-app' : ''}${view === 'engine' || view === 'ledger' || view === 'studio' || view === 'transactions' || view === 'recon' ? ' fit' : ''}`}>
      ...
      <main className="workspace">
        {view === 'engine' ? (
          <div className="engine-grid live-engine">
            <EngineQueueRail
              queue={queue}
              groups={demoGroups}
              ...
              onShowDemos={showDemos}
              onUpload={() => showToast('Receipt upload comes next. Generate one for now.')}
              onSend={runCustom}
              onDemo={runExample}
            />
            <div className="engine-stage">
              <div className="headline">
                <div>
                  <h1>Watch Accounting Happen</h1>
                  <p>Drop a transaction and watch Kanakku evaluate rules, apply treatment, and post.</p>
                </div>
                <div className="headline-tools">
                  <div className="metrics">
                    <div className="metric"><b>{snapshot.metrics.rules}</b><small>Transformations</small></div>
                    <div className="metric"><b>{formatAvg(snapshot.metrics.avgMs)}</b><small>Avg. Processing</small></div>
                    <div className="metric"><b>{snapshot.metrics.balancedPct}%</b><small>Balanced</small></div>
                    <div className="metric"><b>{snapshot.metrics.errors}</b><small>Errors</small></div>
                  </div>
